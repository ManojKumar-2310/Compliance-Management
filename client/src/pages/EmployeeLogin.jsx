...
import { Users, Loader2, ArrowLeft, Mail, Lock as LockIcon, AlertTriangle } from 'lucide-react';
import useAuth from '../hooks/useAuth';
import { useToast } from '../context/ToastContext';
import ThemeToggle from '../components/ThemeToggle';

const EmployeeLogin = () => {
    console.log('EmployeeLogin Component Mounting...');
    ...
    return (
        <div className="min-h-screen flex bg-background text-foreground transition-all duration-500">
            {/* Left Branding Section - Ultra Professional Emerald */}
            <div className="hidden lg:flex lg:w-1/2 bg-[#071410] dark:bg-slate-950 items-center justify-center relative overflow-hidden p-12 lg:p-24 border-r border-white/5">
                <div className="absolute inset-0">
                    <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] bg-emerald-600/20 rounded-full blur-[120px] animate-pulse" />
                    <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-green-600/10 rounded-full blur-[120px]" />
                </div>
                ...
                <div className="relative z-10 max-w-lg">
                    ...
                    >
                        <Users className="text-emerald-500" size={48} />
                    </motion.div>

                    <h1 className="text-6xl font-black text-white mb-8 tracking-tighter leading-none">
                        OPERATIONS <br />
                        <span className="bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-green-400">FRONT-LINE</span>
                    </h1>

                    <p className="text-slate-400 text-xl leading-relaxed mb-12 font-medium">
                        Execute compliance protocols, submit mission-critical evidence, and maintain institutional integrity.
                    </p>

                    <div className="space-y-6">
                        {[
                            { title: 'Task Execution', desc: 'Direct access to assigned operational workflows.' },
                            { title: 'Evidence Log', desc: 'High-speed document and media submission engine.' },
                            { title: 'Progress Matrix', desc: 'Real-time tracking of your compliance standing.' }
                        ].map((item, i) => (
                            <motion.div
                                key={i}
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-8 lg:p-24 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-emerald-50/50 dark:bg-emerald-950/10 rounded-full blur-[100px] -z-10 translate-x-1/2 -translate-y-1/2" />
                ...
                <div className="w-full max-w-md animate-fade-in">
                    <Link
                        to="/"
                        className="inline-flex items-center gap-2 text-slate-400 hover:text-emerald-600 font-bold transition-all mb-12 group"
                    >
                    ...
                    <div className="mb-12">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 font-medium tracking-tight">
                            Connect to the compliance grid and synchronize tasks.
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="space-y-1.5">
                            ...
                            <div className="relative group">
                                ...
                                <input
                                    ...
                                    className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-5 pl-14 rounded-[1.5rem] focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500/50 text-slate-900 dark:text-white font-bold transition-all outline-none"
                                    placeholder="employee@compliance.pro"
                                    required
                                />
                            </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="relative group">
                                <LockIcon className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-emerald-500 transition-colors" size={20} />
                                <input
                                    type="password"
                                    value={password}
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="bg-emerald-600 hover:bg-slate-900 dark:hover:bg-emerald-500 text-white w-full py-5 rounded-[1.5rem] font-black uppercase tracking-[0.2em] text-xs shadow-2xl shadow-emerald-200 dark:shadow-none transition-all hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50"
                        >
                            {loading ? (
                                <>
